...
Survey <- read_csv('/Users/yuanyuanlin/Desktop/mssp/MA615/615/Midterm/Survey.csv')

sur1 <- Survey %>% slice(20:50)
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Methdology", tabName = "data", icon = icon("info")),
      menuSubItem("Exploratory Data Analysis", tabName = "rawdata", icon = icon("database")),
      menuSubItem("Factorial Analysis", tabName = "plot_rawdata", icon = icon("database")),
      menuItem("Result&Limitation", tabName = "Survey", icon = icon("area-chart")),
      menuItem("Conclusion&Implication", tabName = "result", icon = icon("file")),
...
server <-function(input, output) {
  output$diag<-renderPlot({
    ...
    output$head<-renderTable({head(Survey1)})
    
    output$factor<-renderPlot({paralle<-fa.parallel(Survey,fm='minres',fa='fa')})
